    ...
    public static partial class NetworkServer
    {
        ...
        internal static void NetworkLateUpdate()
        {
            ...
            if (active)
            {
                ...
                //if (Time.timeAsDouble >= actualTickRateStart + 1)  commented b smit
                // NetworkTime.localTime has defines for 2019 / 2020 compatibility  as Time.timeAsDouble is not compatible with unity 2019/2020
                if (NetworkTime.localTime >= actualTickRateStart + 1) // found solution for unity 2019 from mirror Github, added by smit
                {
                    // calculate avg by exact elapsed time.
                    // assuming 1s wouldn't be accurate, usually a few more ms passed.
                    float elapsed         = (float)(NetworkTime.localTime - actualTickRateStart); //Replaced Time.timeAsDouble with NetworkTime.localTime by smit
                    actualTickRate        = Mathf.RoundToInt(actualTickRateCounter / elapsed);
                    actualTickRateStart   = NetworkTime.localTime; //Replaced Time.timeAsDouble with NetworkTime.localTime by smit
