...
namespace Mirror
{
    ...
    public class RemoteStatistics : NetworkBehaviour
    {
        ...
        void UpdateServer()
        {
            ...
            //if (Time.timeAsDouble >= lastSendTime + sendInterval) // commented by smit because Time.timeAsDouble is not compatible with unity 2019
            // NetworkTime.localTime has defines for 2019 / 2020 compatibility
            if (NetworkTime.localTime >= lastSendTime + sendInterval) //Added by smit
            {
                ...
                TargetRpcSync(new Stats(
                    ...
                    //Time.realtimeSinceStartupAsDouble,         commented by smit
                    NetworkTime.time,  //Replaced Time.realtimeSinceStartAsDouble with NetworkTime.Time by smit
